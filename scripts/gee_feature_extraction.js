...
var customCounts = [700, 100, 700, 100, 100, 100]; 
...
// =============================================================
// 10. FIGURE 1: VARIABLE IMPORTANCE (RF, GBM, CART)
// =============================================================

// Helper Function to Create and Print Importance Charts
var plotImportance = function(classifier, modelName) {
  
  // 1. Get the Importance Dictionary
  var importanceDict = ee.Dictionary(classifier.explain().get('importance'));
  
  // 2. Sort items to get the Top 15 (Critical for a clean paper figure)
  // We convert to two arrays (keys and values) and sort by values
  var keys = importanceDict.keys();
  var values = importanceDict.values();
  
  // Create an array of [value, key] pairs to sort them together
  var keyValues = ee.Array.cat([ee.Array(keys), ee.Array(values)], 1);
  
  // Sort logic is a bit complex in GEE, so we use a simpler client-side limit for display
  // or a server-side chart that renders everything. 
  // Here is the most robust method that ALWAYS renders:
  ...
  .setOptions({
    title: modelName + ' Variable Importance (Top Features)',
    hAxis: {title: 'Importance Score'},
    vAxis: {title: 'Feature Name', textStyle: {fontSize: 10}},
    height: 500, // Tall enough to read labels
    legend: {position: 'none'},
    colors: ['1a73e8'] // Scientific Blue
  });
  
  print('Generating Chart for ' + modelName + '...');
  print(chart);
};

print('---------------- FIGURE 1: VARIABLE IMPORTANCE ----------------');
print('Note: SVM and MinDist do not support variable importance.');

// 1. Random Forest Importance (The main figure for your paper)
plotImportance(rf, 'Random Forest');

// 2. GBM Importance (For comparison)
plotImportance(gbm, 'Gradient Boosting');

// 3. CART Importance (Baseline)
plotImportance(cart, 'CART');




// =============================================================
// 11. VISUALIZING TRAINING SAMPLES
// =============================================================
...
Export.table.toDrive({
  ...
  description: 'Suam_Training_Points_CP2',
  folder: 'GEE_Exports',
  fileFormat: 'CSV',
...
// =============================================================
// 15. EXTRACT VALUES & EXPORT TO DRIVE (FIXED)
// =============================================================

// 1. Sample the 82-band stack at your training point locations
var sampledPoints = finalStack.sampleRegions({
  collection: training, // Ensure this matches your point variable name
  properties: ['Map'],      // We updated this to 'Map' based on your data!
  scale: 30,                
  tileScale: 16,            
  geometries: false         // This removes the .geo column to keep the CSV clean
});

// 2. Export to Drive
Export.table.toDrive({
  collection: sampledPoints,
  description: 'Suam_Training_Points_Region_CP2',
  folder: 'GEE_Exports', 
  fileFormat: 'CSV'
});
